...
Nota correlata: il backup di configurazione "THENC" (vedi
`root/import_config.py`) usa una chiave AES-256/HMAC-SHA1 separata, letta a
runtime da `/proc/rip/0108` — stesso sotto-sistema `rip`/`ripdrv`, ma
verosimilmente un materiale chiave distinto da quello usato per i `.rbi`.
