...
Analisi estesa a tutte le **14 immagini uniche** trovate in cartella (dedup per hash MD5 — i file `*_copy.rbi` sono duplicati byte-per-byte, esclusi): `1.0.3`, `1.0.4`, `1.1.2`, `1.1.3`, `1.2.0_001`, `2.0.0`, `2.0.0_002`, `2.0.1_003`, `2.2.0`, `2.2.1`, `2.3.2`, `2.4.1`, `2.4.5`, `2.4.5_PATCHED`. Rootfs riestratti nativamente in WSL (ext4, case-sensitive) — un'estrazione su filesystem Windows/DrvFs perde silenziosamente file che differiscono solo per maiuscole/minuscole, quindi ricontrollare sempre su un filesystem case-sensitive prima di trarre conclusioni da un `diff -rq`.

### 3.1 Verifica firma — universale su tutte e 14

Il controllo byte-per-byte descritto al §2.2 (`hash 0xB8 == SHA-256(chunk 0xB4)`) è stato ripetuto su tutte e 14 le immagini, patch inclusa: **match positivo su tutte**, nessuna eccezione. Conferma definitiva che il meccanismo non è mai stato una firma a chiave privata in nessuna versione della linea AGTEF osservata.
...
- **`1.1.3` è un'anomalia netta**: unica versione, in tutta la linea, con struttura di configurazione a istanza singola (stile identico solo a `1.0.3`, non alle versioni adiacenti `1.1.2`/`1.2.0_001` che usano già lo schema `lan`+`wan`) **e** con SSH root abilitato di fabbrica. Non ha nemmeno la cartella `etc/boards/` (assente anche in `1.0.3`, presente in ogni altra versione da `1.0.4` in poi) né `mosquitto` installato. Tutti questi segnali indicano che `AGTEF_1.1.3_CLOSED.rbi` non è una release pubblica TIM standard ma verosimilmente un'immagine di laboratorio/debug — da trattare con cautela se usata per un downgrade reale.
...
| **1.1.3** | **`::askconsole:/bin/ash`** (**attiva, nessun login**) |
| 2.3.2 → 2.4.5_PATCHED | `#::askconsole:/bin/restricted_shell` (disabilitata, ma ora punta a una shell ristretta invece di `/bin/login`) |

Solo `1.1.3` ha la console realmente attiva senza autenticazione — coerente con l'ipotesi di build di laboratorio.

### 3.4 Kernel e board supportate

| Versione | Kernel | Board in `etc/boards/` |
|---|---|---|
| 1.0.3 → 2.0.1_003 | `3.4.11` (+ `3.4.11-rt19`) | 2 (`VBNT-K`, `VBNT-S`) — assente in 1.0.3/1.1.3 |
...
| 1.0.3 / 1.0.4 | ❌ | ❌ | ❌ | ❌ |
| 1.1.2 → 2.0.1_003 | ✅ | ❌ | ❌ | ❌ |
| 1.1.3 | ❌ *(anomalo anche qui)* | ❌ | ❌ | ❌ |
| 2.2.0 → 2.4.5_PATCHED | ✅ | ✅ | ❌ | ❌ |

**WireGuard non è mai presente in nessuna release stock**, su nessuna delle 14 versioni analizzate — coerente con il motivo per cui in questo repository è stato necessario compilare un modulo kernel WireGuard custom (vedi [`GUIDA-IT.md`](GUIDA-IT.md)) invece di attivarne uno già incluso.
...
sottoinsieme dei 14 file unici del confronto in §3): il campo `data_size` (0x2C, 4 byte) e il campo
`counter` del chunk 0xB7 (i primi 4 byte subito dopo `flag`, a offset
`data_offset + 6`) hanno una relazione fissa fra i rispettivi ultimi byte:
...
- Boot Linux completo e pulito con l'immagine appena scritta: nessun kernel
  panic, WiFi (Quantenna) operativo, nessun loop di riavvio.
...
dal log seriale), il login SSH `root`/`root` sulla nuova immagine **continua
a fallire** ("Permission denied (password)"), nonostante l'hash in
`/etc/shadow` sia verificato correttamente in locale prima del flash.
Ipotesi principale, non ancora verificata: questi sistemi montano lo
squashfs in sola lettura con un **overlay scrivibile persistente** (altra
partizione, non toccata dal flash BOOTP/TFTP, che copre solo kernel+rootfs)
per `/etc` — se una versione precedente dei file (da un tentativo di rooting
precedente su questo stesso router) è già stata scritta nell'overlay, questa
oscura le modifiche fatte nello squashfs indipendentemente da cosa contenga
l'immagine appena flashata. Da verificare con un vero factory-reset (7
secondi sul tasto reset **a router già acceso**, procedura diversa dal
power-on-con-reset-premuto usato per la recovery BOOTP) per svuotare
l'overlay. Non confermato al momento della stesura di questa nota. Una
sessione parallela segnala inoltre che flashare `AGTEF_1.0.3` via questo
stesso percorso BOOTP/TFTP produce il sintomo "webUI serve Lua grezzo non
eseguito" già osservato in precedenza durante il flash via BOOTP/TFTP di `AGTEF_1.0.3` (non ancora documentato in questo repo) — riprodotto ora anche con
`221`, il che suggerisce che il sintomo possa dipendere dalla **procedura**
di recovery stessa (mancata re-inizializzazione di qualcosa al primo boot
dopo un flash via questa via) più che dal contenuto specifico della singola
